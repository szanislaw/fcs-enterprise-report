...
def standardize_columns(df):
    rename_map = {
        "room": "room_number",
        "uuid ": "uuid",
        "uuid": "staff_uuid",  # Only effective before knowing file context
        "staff id": "staff_id",
        ...
        "credit": "job_weight",  # renamed to avoid confusion with 'pay'
        "created_date": "date_time_created",
        "date_time_created_": "date_time_created"
    }
    cleaned_cols = []
...
# ─────────────────────────────────────────────────────────────
# 0.1 Helper: Normalize datetime columns
# ─────────────────────────────────────────────────────────────
...
for key, path in csv_files.items():
    if os.path.exists(path):
        ...
        df = normalize_datetime_columns(df)  # NEW — normalise all datetime fields

        # Rename uuid to staff_uuid only for payroll
        if key == "payroll" and "staff_uuid" not in df.columns and "uuid" in df.columns:
...
# ─────────────────────────────────────────────────────────────
# 6. Staff Table (Based on Cleaning Orders and SRs)
# ─────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────
# 7. Checklist & Tags Normalization
# ─────────────────────────────────────────────────────────────
...
print("✅ Normalized SQLite database created: master.db with standardised datetime formats")
