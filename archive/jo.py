...
# ─────────────────────────────────────────────────────────────────────────────
with open("prompt/jo.txt", "r", encoding="utf-8") as f:
    prompt_template = f.read()

# ─────────────────────────────────────────────────────────────────────────────
# Connect to SQLite DB
# ─────────────────────────────────────────────────────────────────────────────
conn = sqlite3.connect("db/jo-payroll.db")

# ─────────────────────────────────────────────────────────────────────────────
# Schema Fetching with PK/FK/Indexes
# ─────────────────────────────────────────────────────────────────────────────
@st.cache_data
def get_schema():
    tables = pd.read_sql(
    ...
    for table in tables:
        ...
        schema_md += "| Column | Type | PK |\n|--------|------|----|\n"
        for _, col in cols.iterrows():
            pk_flag = "✅" if col["pk"] == 1 else ""
            schema_md += f"| `{col['name']}` | `{col['type']}` | {pk_flag} |\n"
        schema_md += "\n"
        ...
        if not fks.empty:
            ...
            for _, fk in fks.iterrows():
                schema_md += f"- `{fk['from']}` → `{fk['table']}.{fk['to']}`\n"
            schema_md += "\n"

        # Indexes
        idx = pd.read_sql(f"PRAGMA index_list({table});", conn)
        if not idx.empty:
            schema_md += "**Indexes:**\n"
            for _, i in idx.iterrows():
                schema_md += f"- `{i['name']}` (unique={i['unique']})\n"
            schema_md += "\n"
...
def apply_sql_fixes(sql):
    ...
    # Replace EXTRACT with strftime
    sql = re.sub(r"EXTRACT\s*\(\s*MONTH\s+FROM\s+([^)]+?)\s*\)",
                 r"CAST(strftime('%m', \1) AS INTEGER)", sql)
    sql = re.sub(r"EXTRACT\s*\(\s*YEAR\s+FROM\s+([^)]+?)\s*\)",
                 r"CAST(strftime('%Y', \1) AS INTEGER)", sql)

    # Remove PostgreSQL-style casts
